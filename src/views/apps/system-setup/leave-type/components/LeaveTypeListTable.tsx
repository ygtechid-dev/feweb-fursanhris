...
import { defaultLeaveTypeValues, LeaveType } from '@/types/leaveTypes'
import { deleteLeaveType, postLeaveType, updateLeaveType } from '@/services/leaveTypeService'
...
type LeaveTypeWithAction = LeaveType & {
  action?: string
}
...
const LeaveTypeListTable = ({ tableData }: { tableData?: LeaveType[] }) => {
  const { user } = useAuth()
  ...
  const [objectToDelete, setObjectToDelete] = useState<LeaveType | null>(null)
  const [isDeleting, setIsDeleting] = useState<boolean>(false)

  const [editDialogOpen, setEditDialogOpen] = useState(false)
  const [objectToEdit, setObjectToEdit] = useState<LeaveType | null>(null)


  // Hooks
  const { lang: locale } = useParams()
  const { dictionary } = useDictionary()
  const methods = useForm<LeaveType>({
    ...
    () => {
      const visibleColumns: ColumnDef<LeaveTypeWithAction, any>[] = [
        ...
        columnHelper.accessor('days', {
          header: `${dictionary['content'].days} / ${dictionary['content'].year}`,
          cell: ({ row }) => (
            <div className='flex items-center gap-4'>
              <div className='flex flex-col'>
                <Typography color='text.primary' className='font-medium'>
  ...
  const table = useReactTable({
    data: filteredData as LeaveType[],
    columns,
    filterFns: {
      fuzzy: fuzzyFilter
    },
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      const response = await deleteLeaveType(objectToDelete.id)
  ...
  const handleDeleteClick = (user: LeaveType) => {
    setObjectToDelete(user)
    setDeleteDialogOpen(true)
  }

  const handleEditClick = async (object: LeaveType) => {
  ...
  return (
    <>
      <Card>
        <CardHeader title={`${dictionary['content'].leaveType} ${dictionary['content'].list}`} className='pbe-4' />
        ...
        <div className='flex justify-between flex-col items-start md:flex-row md:items-center p-6 border-bs gap-4'>
          ...
          <div className='flex flex-col sm:flex-row max-sm:is-full items-start sm:items-center gap-4'>
            ...
            <Button
              ...
              className='max-sm:is-full'
            >
              {dictionary['content'].addNewLeaveType}
            </Button>
          </div>
        </div>
      ...
      <FormDialog
        ...
        title={dictionary['content'].addNewLeaveType}
        onSubmit={async (data: any) => {
          try {
            const res = await postLeaveType(data)
            if (res.status) {
              swrMutate('/leave-types')
      ...
      >
        <>
          ...
          <QTextField
            ...
            fullWidth
            required
            placeholder={`${dictionary['content'].enter} ${dictionary['content'].title}`}
          ...
          <QTextField
            ...
            fullWidth
            required
            placeholder={`${dictionary['content'].enter} ${dictionary['content'].days}`}
            label={`${dictionary['content'].days} / ${dictionary['content'].year}`}
          />
        </>
      </FormDialog>
      ...
      <FormDialog
        ...
        onSubmit={async (data: any) => {
          try {
            ...
            const res = await updateLeaveType(data, objectToEdit.id)
            if (res.status) {
            ...
            toast.error(dictionary['content'].errorUpdating)
          } finally {
            methods.reset()
      ...
      >
        <>
          ...
          <QTextField
            ...
            fullWidth
            required
            placeholder={`${dictionary['content'].enter} ${dictionary['content'].branch} ${dictionary['content'].title}`}
            label={dictionary['content'].title}
          />
          <QTextField
            name='days'
            ...
            fullWidth
            required
            placeholder={`${dictionary['content'].enter} ${dictionary['content'].branch} ${dictionary['content'].days}`}
            label={`${dictionary['content'].days} / ${dictionary['content'].year}`}
          />
        </>
      </FormDialog>
